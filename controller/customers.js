...
const createCustomer = async (req = request, res = response) => {
  try {
    ...
    if (!verifyEmail) {
      // todo: email no existe, por lo tanto se procede a registrar
        ...
        const envioContabilidad = transporter.sendMail(
          {
            ...
            to: [
              "developer@distribuidorauniversalcali.com",
              "mercadeo@distribuidorauniversalcali.com",
            ],
      ...
      return handleErrorResponse(
        res,
        "Ya existe un registro en nuestro sistema relacionado con este correo electrónico",
        404
      );
    }
...
const getCustomers = async (req = request, res = response) => {
  try {
    ...
    res.send({
      data,
      ok: true,
      message: "Has obtenido la lista de los clientes",
    });
  } catch (error) {
    handleHttpError(res, error);
  }
...
const updateCustomer = async (req = request, res = response) => {
  try {
    ...
    res.send({
      token,
      ok: true,
      message: "Has actualizado el cliente",
    });
  } catch (error) {
